...
인앱 토스트를 이미 본 사용자에게 푸시가 또 가면 안 된다. 다만 "지금 화면을 보고 있는가"는 서버가 확실히 알 수 없으므로, 다음 규칙으로 근사한다.

- 신호의 `readAt` 이 채워지면 이미 전달된 것으로 본다. 푸시 발송은 `sentAt` 으로부터 짧은 유예(`PUSH_DELAY_MS`, 기본 4초) 뒤에 `readAt` 이 여전히 `null` 인 신호만 대상으로 한다.
- 유예 대기는 route handler 의 `after()` 안에서 하지 않는다. 서버리스에서 함수가 먼저 끝날 수 있다. 대신 발송 시점에 한 번 더 `readAt` 을 확인하는 방식으로, 이미 읽힌 신호는 건너뛴다.
