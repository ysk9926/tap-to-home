 ...
 *      정산돼 있으면 예외를 던진다. 집까지 남은 횟수는 lock 을 쥔 상태에서 계산한다.
...
export async function recordTaps(
  ...
  return prisma.$transaction(async (tx) => {
    const run = await tx.dailyRun.upsert({
      ...
      // A real UPDATE (even +0) holds the row lock until the transaction commits.
      update: { tapCount: { increment: 0 } },
      select: { id: true, tapCount: true, stage: true, firstTapAt: true },
    });
    ...
    await tx.tapEvent.create({ data: { dailyRunId: run.id, tappedAt: now, batchSize: accepted } });
    await tx.dailyRun.update({
      where: { id: run.id },
      data: { tapCount, stage, firstTapAt: run.firstTapAt ?? now, lastTapAt: now },
    });

    return { tapCount, stage };
  });
}
