...
beforeEach(() => {
  ...
  vi.stubGlobal("fetch", async (_input: unknown, init: RequestInit) => {
    ...
    return new Response(JSON.stringify({ tapCount: 5000, stage: 5, date: "2026-09-20" }));
  });
});
...
it("accepts only the last tap even when multiple taps happen before rerender", async () => {
  ...
  expect(client.getQueryData<RaceToday>(raceTodayKey("me"))?.me.tapCount).toBe(5000);
...
it("does not let yesterday's final response lock a new day's race", async () => {
  ...
  await act(async () => { finish(new Response(JSON.stringify({ tapCount: 5000, stage: 5, date: "2026-09-20" }))); });
...
it.each([{ count: 5000, settled: false }, { count: 6000, settled: false }, { count: 50, settled: true }])(
